    ...
    >
{
    ...
    pub fn load_file_bytes(&mut self, inode: &Inode<MAX_FILE_BLOCKS, BLOCK_SIZE>) {
        for i in 0..self.file_content_buffer.len() {
            self.file_content_buffer[i] = 0;
        }
        for i in 0..inode.blocks_used() {
            let block_num: usize = inode.blocks[i] as usize;
            self.disk.read(block_num, &mut self.block_buffer).unwrap();
            let start: usize = i * BLOCK_SIZE;
            let bytes_to_copy: usize = min(BLOCK_SIZE, inode.bytes_stored as usize - start);
            if bytes_to_copy > 0 {
                self.file_content_buffer[start..start + bytes_to_copy]
                    .copy_from_slice(&self.block_buffer[..bytes_to_copy]);
            }
        }
    }
    ...
    pub fn save_file_bytes(&mut self, inode: &Inode<MAX_FILE_BLOCKS, BLOCK_SIZE>) {
        for i in 0..inode.blocks_used() {
            ...
            let bytes_to_copy: usize = min(BLOCK_SIZE, inode.bytes_stored as usize - start);
            if bytes_to_copy > 0 {
                self.block_buffer[..bytes_to_copy]
                    .copy_from_slice(&self.file_content_buffer[start..start + bytes_to_copy]);
            }
    ...
    pub fn activate_bit(&mut self, bit: usize, block: usize) {
        ...
        let bit_to_set: usize = bit % 8;
        self.block_buffer[byte] |= 1 << bit_to_set;
    ...
    fn request_data_block(&mut self) -> Result<u8, FileSystemError> {
        ...
        if let Some(block) = lowest_zero_in(&self.block_buffer) {
            if block >= NUM_BLOCKS {
    ...
    pub fn inode_table_inode(&mut self) -> Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> {
        let mut table: [u8; MAX_FILE_BLOCKS] = [0; MAX_FILE_BLOCKS];
        for (i, block_num) in (2..self.first_data_block()).enumerate() {
            table[i] = block_num as u8;
        }
    ...
    fn save_inode(&mut self, inode_num: usize, inode: &Inode<MAX_FILE_BLOCKS, BLOCK_SIZE>) {
        let inode_table: Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> = self.inode_table_inode();
    ...
    fn load_inode(&mut self, inode_num: usize) -> Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> {
        let inode_table: Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> = self.inode_table_inode();
        self.load_file_bytes(&inode_table);
        Inode::from_bytes(&self.file_content_buffer[inode_num * self.num_inode_bytes()..])
    }
    ...
    fn initialize_new_file(
        ...
        let inode: Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> = Inode::new(first_block);
    ...
    fn directory_inode(&mut self) -> Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> {
        match self.directory_exists() {
            true => return self.load_inode(0),
            false => {
                self.mark_inode_blocks_in_use();
                return self.initialize_new_file(0, self.first_data_block() as u8);
            }
        }
    }
    ...
    fn load_directory(&mut self) -> Result<(), FileSystemError> {
        match self.directory_exists() {
            true => {
                let dir_inode: Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> = self.directory_inode();
                self.load_file_bytes(&dir_inode);
                Ok(())
            },
    ...
    {
        if let Some(lowest_zero_bit) = self.find_lowest_zero_bit_in(INODE_FULL_BLOCK) {
            if lowest_zero_bit == 0 {
                return Err(FileSystemError::DirectoryEmpty);
            }
            let num_files: usize = lowest_zero_bit - 1;
            self.load_directory()?;
            ...
            return Ok((num_files, directory_array));
        }
    ...
    pub fn inode_for(
        ...
        for i in 0..MAX_FILES_STORED {
            ...
            for (j, char) in filename.chars().enumerate() {
                if j >= MAX_FILENAME_BYTES {
                    match_found = false;
                    break;
                }
                let byte: u8 = self.file_content_buffer[offset + j];
                if byte == 0 || byte != char as u8 {
                    match_found = false;
                    break;
                }
            }
            if match_found {
                if filename.len() < MAX_FILENAME_BYTES && self.file_content_buffer[offset + filename.len()] != 0 {
                    match_found = false;
                }
            }
            if match_found {
                let inode_num: usize = i + 1;
                let inode: Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> = self.load_inode(inode_num);
    ...
    pub fn open_read(&mut self, filename: &str) -> Result<usize, FileSystemError> {
        let file_inode: (usize, Inode<MAX_FILE_BLOCKS, BLOCK_SIZE>) = self.inode_for(filename)?;
        if self.open_inodes[file_inode.0] {
            return Err(FileSystemError::AlreadyOpen);
        }
        if let Some(lowest_fd) = self.find_lowest_fd() {
            let mut file_info: FileInfo<MAX_FILE_BLOCKS, BLOCK_SIZE> = FileInfo::read(file_inode.1, file_inode.0);
            self.disk.read(file_inode.1.blocks[0] as usize, &mut file_info.block_buffer).unwrap();
            self.open[lowest_fd] = Some(file_info);
            self.open_inodes[file_inode.0] = true;
            return Ok(lowest_fd);
        }
    ...
    pub fn open_create_new(&mut self, filename: &str) -> Result<usize, FileSystemError> {
        let mut dir_inode: Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> = self.directory_inode();
        let inode_num: usize;
        match self.find_lowest_zero_bit_in(INODE_FULL_BLOCK) {
            Some(num) => {
                if num >= MAX_FILES_STORED {
        ...
        let first_data_block: u8 = self.request_data_block()?;
        let new_file_inode: Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> = self.initialize_new_file(inode_num, first_data_block);
        self.create_directory_entry(filename, inode_num, &mut dir_inode)?;
        if let Some(lowest_fd) = self.find_lowest_fd() {
            let file_info: FileInfo<MAX_FILE_BLOCKS, BLOCK_SIZE> = FileInfo::write(new_file_inode, inode_num);
            self.open[lowest_fd] = Some(file_info);
    ...
    fn create_directory_entry(
        ...
        self.load_directory()?;
        let array_entry: usize = MAX_FILENAME_BYTES + (inode_num - 1) * MAX_FILENAME_BYTES;
        let current_directory_size: usize = directory_inode.bytes_stored as usize;
        let entry_end: usize = array_entry + MAX_FILENAME_BYTES;
        if entry_end > current_directory_size {
            let old_blocks: usize = directory_inode.blocks_used();
            directory_inode.bytes_stored = entry_end as u16;
            let new_blocks: usize = directory_inode.blocks_used();
            if new_blocks > old_blocks {
                let new_block: u8 = self.request_data_block()?;
                directory_inode.blocks[old_blocks] = new_block;
            }
        }
        for (i, char) in filename.chars().enumerate() {
            self.file_content_buffer[array_entry + i] = char as u8;
        }
        if filename.len() < MAX_FILENAME_BYTES {
            for i in filename.len()..MAX_FILENAME_BYTES {
                self.file_content_buffer[array_entry + i] = 0;
            }
        }
    ...
    pub fn close(&mut self, fd: usize) -> Result<(), FileSystemError> {
        let file_info: FileInfo<MAX_FILE_BLOCKS, BLOCK_SIZE> = match self.open[fd] {
            Some(info) => info,
    ...
    pub fn read(&mut self, fd: usize, buffer: &mut [u8]) -> Result<usize, FileSystemError> {
        let mut file_info: FileInfo<MAX_FILE_BLOCKS, BLOCK_SIZE> = match self.open[fd] {
            Some(info) => info,
            None => return Err(FileSystemError::FileNotOpen)
        };
        if file_info.writing {
        ...
        let bytes_read: usize = file_info.current_block * BLOCK_SIZE + file_info.offset;
        let total_bytes: usize = file_info.inode.bytes_stored as usize;
        if bytes_read >= total_bytes {
            return Ok(0);
        }
        let bytes_remaining: usize = total_bytes - bytes_read;
        let bytes_to_read: usize = min(bytes_remaining, buffer.len());
        let mut bytes_read: usize = 0;
        while bytes_read < bytes_to_read {
            if file_info.offset >= BLOCK_SIZE {
                file_info.current_block += 1;
                file_info.offset = 0;
                let block_num: usize = file_info.inode.blocks[file_info.current_block] as usize;
                self.disk.read(block_num, &mut file_info.block_buffer).unwrap();
            }
            let bytes_in_block: usize = BLOCK_SIZE - file_info.offset;
            let bytes_to_copy: usize = min(bytes_in_block, bytes_to_read - bytes_read);
            buffer[bytes_read..bytes_read + bytes_to_copy].copy_from_slice(
                &file_info.block_buffer[file_info.offset..file_info.offset + bytes_to_copy]
            );
    ...
    pub fn write(&mut self, fd: usize, buffer: &[u8]) -> Result<(), FileSystemError> {
        ...
        for &byte in buffer {
            if file_info.offset >= BLOCK_SIZE {
                self.disk.write(file_info.inode.blocks[file_info.current_block] as usize, 
                              &file_info.block_buffer).unwrap();
                file_info.current_block += 1;
                file_info.offset = 0;
                ...
                let new_block: u8 = self.request_data_block()?;
                file_info.inode.blocks[file_info.current_block] = new_block;
            }
            file_info.block_buffer[file_info.offset] = byte;
            file_info.offset += 1;
            let bytes_written: usize = file_info.current_block * BLOCK_SIZE + file_info.offset;
            if bytes_written > file_info.inode.bytes_stored as usize {
                file_info.inode.bytes_stored = bytes_written as u16;
            }
        }
        self.disk.write(file_info.inode.blocks[file_info.current_block] as usize, 
                      &file_info.block_buffer).unwrap();
    ...
    pub fn open_create_reset(
        ...
        for i in 0..inode.blocks_used() {
            let block_num: usize = inode.blocks[i] as usize;
            let byte_index: usize = block_num / 8;
            let bit_position: usize = block_num % 8;
            self.block_buffer[byte_index] &= !(1 << bit_position);
        }
        let new_first_block: u8 = self.request_data_block()?;
        self.disk.write(new_first_block as usize, &self.block_buffer).unwrap();
        inode.bytes_stored = 0;
        inode.blocks[0] = new_first_block;
        self.save_inode(inode_num, &inode);
        if let Some(lowest_fd) = self.find_lowest_fd() {
            let file_info: FileInfo<MAX_FILE_BLOCKS, BLOCK_SIZE> = FileInfo::write(inode, inode_num);
            self.open[lowest_fd] = Some(file_info);
            self.open_inodes[inode_num] = true;
            return Ok(lowest_fd);
        }
    ...
    pub fn open_append(&mut self, filename: &str) -> Result<usize, FileSystemError> {
        ...
        if let Some(lowest_fd) = self.find_lowest_fd() {
            let current_block: usize = file_inode.1.bytes_stored as usize / BLOCK_SIZE;
            let offset: usize = file_inode.1.bytes_stored as usize % BLOCK_SIZE;
            let mut file_info: FileInfo<MAX_FILE_BLOCKS, BLOCK_SIZE> = FileInfo::write_inside(
                file_inode.1,
                file_inode.0,
                current_block,
                offset
            );
            self.disk.read(file_inode.1.blocks[current_block] as usize, &mut file_info.block_buffer).unwrap();
            self.open[lowest_fd] = Some(file_info);
            self.open_inodes[file_inode.0] = true;
            return Ok(lowest_fd);
        }
...
impl<const MAX_BLOCKS: usize, const BLOCK_SIZE: usize> Inode<MAX_BLOCKS, BLOCK_SIZE> {
    ...
    pub fn from_bytes(buffer: &[u8]) -> Self {
        ...
        let shifted_high_bits: u16 = (buffer[1] as u16) << 8;
        let bytes_stored: u16 = shifted_high_bits | (buffer[0] as u16);
        let inode: Inode<MAX_BLOCKS, BLOCK_SIZE> = Inode {
            bytes_stored,
            blocks: {
                let mut blocks: [u8; MAX_BLOCKS] = [0; MAX_BLOCKS];
                blocks.copy_from_slice(&buffer[2..2 + MAX_BLOCKS]);
                blocks
            }
        };
        inode
